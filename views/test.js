...
function interceptForm(event) {
    ...
    const beginDate = new Date(form[2].value);
    const endDate = new Date(form[3].value);
    ...
    if (title.lenght === 0) {
        txtToComplete += ' le titre de l\'évenement';
        complete = false;
    }
    if (location.lenght === 0) {
        txtToComplete += ' la localisation';
        complete = false;
    }
    ...
    if (complete) {
        ...
        message.innerHTML = txtToComplete;
        return null;
    }
}
